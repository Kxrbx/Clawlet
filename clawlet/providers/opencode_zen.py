...
class OpenCodeZenProvider(BaseProvider):
    ...
    def __init__(
        ...
        if not api_key or api_key == _API_KEY_PLACEHOLDER:
            raise ValueError(
                "OpenCode Zen API key is required. Set it via:\n"
                "  1. Environment variable: export OPENCODE_ZEN_API_KEY=your_key\n"
                "  2. Or in ~/.clawlet/config.yaml under provider.opencode_zen.api_key"
            )
        ...
        logger.info(f"OpenCode Zen provider initialized with model={default_model}")
    ...
    async def complete(
        ...
        **kwargs
        ...
        logger.info(f"OpenCode Zen request: model={model}, messages={len(messages)}")
        
        try:
            logger.debug(f"Sending request to OpenCode Zen API...")
            response = await client.post("/chat/completions", json=payload)
            logger.debug(f"OpenCode Zen response received: status={response.status_code}")
            ...
            logger.info(f"OpenCode Zen response: {len(content)} chars, {usage.get('total_tokens', 0)} tokens")
            ...
            logger.error(f"OpenCode Zen HTTP error: {e.response.status_code} - {e.response.text}")
            raise
    ...
    async def stream(
        ...
        **kwargs
        ...
        logger.debug(f"OpenCode Zen stream request: model={model}")
        ...
        try:
            ...
            logger.error(f"OpenCode Zen stream error: {e}")
            raise
