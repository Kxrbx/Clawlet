...
class ZAIProvider(BaseProvider):
    ...
    def __init__(
        ...
        if not api_key or api_key == _API_KEY_PLACEHOLDER:
            raise ValueError(
                "Z.AI API key is required. Set it via:\n"
                "  1. Environment variable: export ZAI_API_KEY=your_key\n"
                "  2. Or in ~/.clawlet/config.yaml under provider.zai.api_key"
            )
        ...
        logger.info(f"Z.AI provider initialized with model={default_model}")
    ...
    async def complete(
        ...
        **kwargs
        ...
        logger.info(f"Z.AI request: model={model}, messages={len(messages)}")
        
        try:
            logger.debug(f"Sending request to Z.AI API...")
            response = await client.post("/chat/completions", json=payload)
            logger.debug(f"Z.AI response received: status={response.status_code}")
            ...
            logger.info(f"Z.AI response: {len(content)} chars, {usage.get('total_tokens', 0)} tokens")
    ...
    async def stream(
        ...
        **kwargs
        ...
        logger.debug(f"Z.AI stream request: model={model}")
    ...
    async def list_models(self, force_refresh: bool = False) -> List[dict]:
        """List all available models from Z.AI.
        
        Returns a list of model objects with id, name, and other metadata.
        Since Z.AI doesn't have a public model listing API, returns DEFAULT_MODELS.
        """
