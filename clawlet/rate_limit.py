...
class RateLimiter:
    ...
    def __init__(
        ...
        logger.info(
            f"RateLimiter initialized: messages={self.default_limit}, tools={self.tool_limit}, max_entries={self.max_entries}"
        )
    ...
    def is_allowed(
        ...
        if is_new_key:
            if not self._ensure_capacity(key, now):
                logger.error(
                    f"Rate limiter cannot accept new key {key}: too many entries"
                )
    ...
    def _cleanup(self, aggressive: bool = False, now: Optional[float] = None) -> None:
        ...
        current_time = time.time() if now is None else now
        max_window = max(
            self.default_limit.window_seconds, self.tool_limit.window_seconds
        )

        batch_size = (
            self._aggressive_cleanup_batch_size
            if aggressive
            else self._cleanup_batch_size
        )
